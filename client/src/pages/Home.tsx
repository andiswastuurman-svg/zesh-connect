...
const creators: Creator[] = [
  { name: "Mila Jacobs", handle: "@milamakeswaves", niche: "Travel & Lifestyle", location: "Sea Point", followers: "86.4K", engagement: "6.8%", rate: "R8K – R14K", platforms: ["Instagram", "TikTok", "Facebook"], initials: "MJ", tone: "coral", match: 96 },
  { name: "Thabo Ndlovu", handle: "@thaboeatscape", niche: "Food & Hospitality", location: "Observatory", followers: "42.1K", engagement: "8.4%", rate: "R5K – R9K", platforms: ["Instagram", "YouTube", "Twitter/X"], initials: "TN", tone: "blue", match: 92 },
  { name: "Zinhle Mbeki", handle: "@zinhlecreates", niche: "Beauty & Culture", location: "Woodstock", followers: "118K", engagement: "5.1%", rate: "R12K – R22K", platforms: ["Instagram", "TikTok", "YouTube"], initials: "ZM", tone: "teal", match: 89 },
  { name: "Ethan Cole", handle: "@ethanonthegrid", niche: "Tech & Business", location: "Gardens", followers: "29.8K", engagement: "7.2%", rate: "R4K – R8K", platforms: ["LinkedIn", "Instagram", "Twitter/X"], initials: "EC", tone: "navy", match: 84 },
  { name: "Aaliyah Daniels", handle: "@aaliyahafterdark", niche: "Fashion & Beauty", location: "Claremont", followers: "63.7K", engagement: "9.1%", rate: "R7K – R13K", platforms: ["Instagram", "TikTok", "Facebook"], initials: "AD", tone: "plum", match: 81 },
  { name: "Luca van Wyk", handle: "@lucaexplores", niche: "Outdoors & Travel", location: "Hout Bay", followers: "51.3K", engagement: "6.2%", rate: "R6K – R11K", platforms: ["Instagram", "YouTube", "Twitter/X"], initials: "LV", tone: "green", match: 78 },
];
...
export default function Home() {
  ...
  const filteredCreators = useMemo(() => creators.filter((creator) => {
    ...
    return searchable.includes(search.toLowerCase()) && (niche === "All niches" || creator.niche === niche) && (platform === "All platforms" || creator.platforms.includes(platform));
  ...
  return <div className="app-shell">
    ...
    <main className="main-canvas">
      ...
      <div className="content-wrap">
        ...
        {activeView === "Discover creators" && <CreatorDirectory filteredCreators={filteredCreators} search={search} setSearch={setSearch} niche={niche} setNiche={setNiche} platform={platform} setPlatform={setPlatform} saved={saved} setSaved={setSaved} notify={notify} onView={setSelectedCreator} />}
    ...
    {selectedCreator && <CreatorProfile creator={selectedCreator} close={() => setSelectedCreator(null)} notify={notify} />}
    {toast && <div className="toast"><Check size={16} /> {toast}</div>}
  </div>;
}

function CreatorDirectory({ filteredCreators, search, setSearch, niche, setNiche, platform, setPlatform, saved, setSaved, notify, onView }: { filteredCreators: Creator[]; search: string; setSearch: (x: string) => void; niche: string; setNiche: (x: string) => void; platform: string; setPlatform: (x: string) => void; saved: string[]; setSaved: (x: string[]) => void; notify: (x: string) => void; onView: (creator: Creator) => void }) {
  return <div className="directory-view"><div className="directory-toolbar"><div className="search-field"><Search size={17} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search creators, niches or locations" /></div><select value={niche} onChange={(e) => setNiche(e.target.value)}><option>All niches</option><option>Travel & Lifestyle</option><option>Food & Hospitality</option><option>Beauty & Culture</option><option>Tech & Business</option><option>Fashion & Beauty</option><option>Outdoors & Travel</option></select><select value={platform} onChange={(e) => setPlatform(e.target.value)}><option>All platforms</option><option>Instagram</option><option>TikTok</option><option>LinkedIn</option><option>Facebook</option><option>Twitter/X</option></select><button className="filter-button"><Filter size={16} /> More filters</button></div><div className="directory-meta"><div><strong>{filteredCreators.length * 74 + 312}</strong> creators in Cape Town <span className="live-dot" /> Updated today</div><button className="ai-recommendation" onClick={() => notify("AI recommendations refreshed") }><Sparkles size={15} /> AI recommendations <ArrowUpRight size={14} /></button></div><div className="creator-grid">{filteredCreators.map((creator) => <CreatorCard key={creator.name} creator={creator} isSaved={saved.includes(creator.name)} onSave={() => setSaved(saved.includes(creator.name) ? saved.filter((name) => name !== creator.name) : [...saved, creator.name])} onView={() => onView(creator)} notify={notify} />)}</div>{filteredCreators.length === 0 && <div className="empty-state"><Search size={24} /><h3>No creators found</h3><p>Try a broader search or reset your niche filter.</p></div>}</div>;
}

function CreatorCard({ creator, isSaved, onSave, onView, notify }: { creator: Creator; isSaved: boolean; onSave: () => void; onView: () => void; notify: (x: string) => void }) {
  return <article className="creator-card"><div className={`creator-cover cover-${creator.tone}`}><div className="cover-grid" /><button className={`save-button ${isSaved ? "saved" : ""}`} onClick={onSave}><Heart size={16} fill={isSaved ? "currentColor" : "none"} /></button><div className="creator-avatar-wrap"><Avatar initials={creator.initials} tone={creator.tone} size="lg" /><span className="verified"><Check size={11} /></span></div></div><div className="creator-card-body"><div className="creator-name-row"><div><h3>{creator.name}</h3><span>{creator.handle}</span></div><span className="match-score"><Sparkles size={12} /> {creator.match}%</span></div><p className="creator-niche">{creator.niche}</p><div className="creator-tags">{creator.platforms.map((platform) => <span key={platform}>{platform}</span>)}<span>{creator.location}</span></div><div className="creator-metrics"><div><strong>{creator.followers}</strong><span>Audience</span></div><div><strong>{creator.engagement}</strong><span>Engagement</span></div><div><strong>{creator.rate}</strong><span>Typical rate</span></div></div><button className="button button-outline full-button" onClick={onView}>View profile <ArrowUpRight size={14} /></button></div></article>;
}

function CreatorProfile({ creator, close, notify }: { creator: Creator; close: () => void; notify: (x: string) => void }) {
  return <div className="modal-backdrop"><div className="profile-modal"><div className={`profile-hero cover-${creator.tone}`}><button className="profile-close" onClick={close}><X size={18} /></button><div className="profile-hero-art"><div className="profile-plane profile-plane-one" /><div className="profile-plane profile-plane-two" /></div><div className="profile-avatar"><Avatar initials={creator.initials} tone={creator.tone} size="lg" /><span className="verified"><Check size={11} /></span></div></div><div className="profile-body"><div className="profile-title"><div><h2>{creator.name}</h2><span>{creator.handle} · {creator.location}, Cape Town</span></div><span className="match-score"><Sparkles size={12} /> {creator.match}% fit</span></div><p className="profile-niche">{creator.niche} creator with a warm, editorial point of view and a highly responsive local audience.</p><div className="profile-stats"><div><strong>{creator.followers}</strong><span>Audience</span></div><div><strong>{creator.engagement}</strong><span>Engagement</span></div><div><strong>{creator.rate}</strong><span>Typical rate</span></div></div><div className="profile-columns"><div><p className="eyebrow">Why this match</p><div className="match-reasons"><span><Check size={13} /> Strong lifestyle relevance</span><span><Check size={13} /> Audience overlap: 82%</span><span><Check size={13} /> High save-rate signal</span></div></div><div><p className="eyebrow">Representation</p><p className="agent-note"><Users size={14} /> Managed by <strong>Northstar Talent</strong></p></div></div><div className="profile-actions"><button className="button button-ghost" onClick={() => notify("Creator saved to your shortlist") }><Heart size={15} /> Save shortlist</button><button className="button button-primary" onClick={() => { close(); notify(`Outreach draft started for ${creator.name}`); }}><Mail size={15} /> Start collaboration</button></div></div></div></div>;
}
...
function AIStudio({ aiType, setAiType, aiBrief, setAiBrief, generated, generate, generating, notify }: { aiType: string; setAiType: (x: string) => void; aiBrief: string; setAiBrief: (x: string) => void; generated: string; generate: () => void; generating: boolean; notify: (x: string) => void }) {
  ...
  return <div className="ai-studio"><div className="ai-intro"><div><span className="mini-label"><Sparkles size={14} /> Built for the brief</span><h2>From blank page to <em>brand spark.</em></h2><p>Tell Zesh what you’re building. We’ll help you find the words, hooks and angles that make creators want to say yes.</p></div><div className="ai-orbit"><div className="orbit-ring ring-one" /><div className="orbit-ring ring-two" /><Sparkles size={25} /></div></div><div className="ai-layout"><div className="ai-form panel"><p className="eyebrow">01 · Choose a format</p><div className="tool-grid">{tools.map((tool) => <button key={tool} className={aiType === tool ? "selected" : ""} onClick={() => setAiType(tool)}><span>{tool === "Campaign slogan" ? "✦" : tool === "Social caption" ? "◌" : tool === "Ad concept" ? "▱" : tool === "Video concept" ? "▶" : tool === "Hashtag set" ? "#" : tool === "Influencer brief" ? "☷" : "→"}</span>{tool}</button>)}</div><label className="eyebrow">02 · Give us the brief</label><textarea value={aiBrief} onChange={(e) => setAiBrief(e.target.value)} /><div className="form-foot"><span>{aiBrief.length} / 500</span><button className="button button-primary" onClick={generate} disabled={generating}><WandSparkles size={16} /> {generating ? "Thinking…" : "Generate draft"}</button></div></div><div className="ai-output panel"><div className="output-header"><div><p className="eyebrow">03 · Your draft</p><h3>{aiType}</h3></div><span className="ai-status"><span className="live-dot" /> AI ready</span></div><div className={`output-content ${generated.startsWith("Your AI") ? "empty-output" : ""}`}><Sparkles size={21} />{generated}</div><div className="output-actions"><button onClick={() => { navigator.clipboard?.writeText(generated); notify("Draft copied to clipboard"); }}><Copy size={15} /> Copy</button><button onClick={() => notify("Draft saved to Library") }><Heart size={15} /> Save to library</button></div></div></div><div className="response-strip"><div className="response-icon"><Mail size={19} /></div><div><strong>Need to reach out?</strong><span>Generate a polished collaboration message for any creator.</span></div><button className="button button-outline" onClick={() => setResponseOpen(true)}>Open response generator <ArrowUpRight size={15} /></button>{responseOpen && <ResponseGenerator close={() => setResponseOpen(false)} notify={notify} />}</div></div>;
}

function ResponseGenerator({ close, notify }: { close: () => void; notify: (x: string) => void }) {
  const responseMutation = trpc.ai.generateResponse.useMutation();
  const [creator, setCreator] = useState("Mila Jacobs");
  const [tone, setTone] = useState("Warm and professional");
  const [context, setContext] = useState("We are launching a locally made skincare edit this winter and would love to explore a paid Reel and Stories collaboration.");
  const [draft, setDraft] = useState("");
  return <div className="response-generator"><div className="response-generator-head"><div><p className="eyebrow">Zesh AI · Collaboration response</p><h3>Make the first message count.</h3></div><button onClick={close}><X size={16} /></button></div><div className="response-generator-grid"><div><label className="eyebrow">Creator<input value={creator} onChange={(e) => setCreator(e.target.value)} /></label><label className="eyebrow">Tone<select value={tone} onChange={(e) => setTone(e.target.value)}><option>Warm and professional</option><option>Concise and direct</option><option>Friendly and casual</option></select></label><label className="eyebrow">Campaign context<textarea value={context} onChange={(e) => setContext(e.target.value)} /></label><button className="button button-primary" disabled={responseMutation.isPending} onClick={() => responseMutation.mutate({ creator, tone, context }, { onSuccess: (result) => { setDraft(result.content); notify("Collaboration message drafted"); }, onError: () => notify("Could not draft response") })}><Sparkles size={15} /> {responseMutation.isPending ? "Drafting…" : "Draft response"}</button></div><div className="response-draft"><p className="eyebrow">Draft preview</p><div>{draft || "Your professional outreach message will appear here."}</div>{draft && <button onClick={() => { navigator.clipboard?.writeText(draft); notify("Message copied"); }}><Copy size={14} /> Copy message</button>}</div></div></div>;
}
...
function Library({ imageSearch, setImageSearch, notify }: { imageSearch: string; setImageSearch: (x: string) => void; notify: (x: string) => void }) {
  ...
  const imageMutation = trpc.ai.generateInspiration.useMutation();
  const librarySave = trpc.library.save.useMutation();
